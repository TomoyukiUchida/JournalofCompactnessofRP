...
such that $q$ minimally supports $D$ for a regular pattern $p$. %with $p\not\preceq q$ under some conditions with the symbols $a_i,b_i$ for $i~(1\leq i\leq 3)$.
Obviously, we remark that $D = \{ a_{1}b_{1}, a_{2}b_{2}, a_{3}y\}$ and $D = \{ a_{1}b_{1}, a_{2}b_{2}, yb_{3}\}$ satisfies that $a_{i}\neq a_{3}$ and $b_{i}\neq b_{3}$ for $i=1,2$, respectively.
}
...
\begin{lem}\label{lem:3consts_i}
  ...
  {\color{red} $D=\{ a_{1}b_{1}, a_{2}b_{2}, a_{3}y\}\subsetneq \RPat_{\Sigma\cup X}$
...
%
%Since $D$ is maximally generalized {\color{red}with respect to} $(p,q)$, 
...
\begin{itemize}
  {\color{red} \item $|w|=|w^{\prime}|$: From (1') and (2'), we have $a_2=a_3$.
...
%
\item $|w|+1 < |w^{\prime}|$: From (2) and (3), {\color{red}both }$a_{1}b_{1}wa_{2}b_{2}w^{\prime}$ and $a_{1}b_{1}w$ are suffixes of $p_{1}$.
Hence, {\color{red}$a_{1}b_{1}w$ is the suffix of $w^{\prime}$}.
Moreover, {\color{red}both }$wa_{2}b_{2}w^{\prime}a_{3}$ and $w^{\prime}a_{3}$ are prefixes of $p_{2}$ from (1') and (2').
Hence, there exist constant {\color{red}strings} $w_{1}$ and $w_{2}$ such that $w^{\prime}=w_{1}w$, $w^{\prime}=ww_{2}$ and $|a_{2}b_{2}w_{1}|=|w_{2}a_{3}|+1$ hold.
...
there exists a constant symbol $w_{2}$ such that $w=w_{2}w^{\prime}$ and {\color{red}$w_{1}a_{2}b_{2}w^{\prime}=a_{1}b_{1}w_{2}w^{\prime}$} hold.
...
Moreover, since there exists a constant string $w_{2}$ such that $w=w_{2}w^{\prime}$ and {\color{red}$w_{1}a_{2}b_{2}=a_{1}b_{1}w_{2}$} hold,
...
\end{align*}
\noindent
%Then, it implies a contradiction in the following inductive way:
{\color{red} This leads to a contradiction, as demonstrated by the following inductive argument:}
\begin{itemize}
...
\begin{lem}\label{lem:3consts_ii}
  Let $\Sigma$ be an alphabet $\sharp\Sigma \ge 3$ $p,~q$ regular patterns {\color{red} in $\RPat_{\Sigma\cup X}$ with $p\not\preceq q$ and
  $D=\{ a_{1}b_{1}, a_{2}b_{2}, yb_{3}\} \subsetneq \RPat_{\Sigma\cup X}$
  ...
  Then, 
  %if $p \{ x := r \} \preceq q$ holds for all $r \in D$,
if $q$ minimally supports $D$ for $p$,
then $p \{ x := xy \} \preceq q$ holds.}
  %Let $D$ be the following set of regular patterns {\color{red}over} $\Sigma\cup X$, where $y$ is a variable symbol in $X$ that does not appear in $p$ and $q$:
  %\begin{align*}
...
%Obviously, we remark that $D$ satisfies that $a_i\neq a_j$ and $b_i\neq b_j$ for $i,j~(i\neq j, 1\leq i,j\leq 3)$
%In this subsection, for any regular patterns $p,q$ over $\Sigma\cup X$ with $p\not\preceq q$, 
...
\begin{figure*}[t]
  \begin{center}
    ...
    %In these cases, if $p \{ x := r \} \preceq q$ for all $r \in D$ and $D$ is maximally generalized {\color{red}with respect to} ($p,q$), 
...
\begin{lem}\label{lem:3consts_iii}
  Let $\Sigma$ be an alphabet with $\sharp\Sigma \ge 3$, $p,~q$ regular pattern {\color{red}in $\RPat_{\Sigma\cup X}$ with $p\not\preceq q$} and
  {\color{red} $D=\{ a_{1}b_{1}, a_{2}b_{2}, a_{3}b_{3}\}\subsetneq \RPat_{\Sigma\cup X}$ where $a_{i} \ne a_{j} \mbox{ and } b_{i} \ne b_{j} \mbox{ with } i\ne j ~(1\le i,j\le 3)$.
...
%
\item $|w|+1=|w^{\prime}|$: The {\color{red} two} strings $wa_{2}b_{2}w^{\prime}a_{3}b_{3}$ and $w^{\prime}a_{3}b_{3}$ are prefixes of $p_{2}$.
...
{\color{red}From} (2) and (3), {\color{red}both } $a_{1}b_{1}wa_{2}b_{2}w^{\prime}$ and $a_{1}b_{1}w$ are suffixes of $p_{1}$.
...
If there exist constant strings $w_{1}$ and $w_{2}$ such that $w^{\prime} = w_{1}w=ww_{2}$ holds, then {\color{red} $a_{3}b_{3}$ is a suffix of $w_{1}$ from $|w_1|=|w_2|$ and $ww_{2}a_{3}b_{3}=wa_{2}b_{2}w_{1}$}.
...
Since $b_{2}=a_{1}$ and {\color{red}$b_{2}=a_{3}$, we have $a_{1}=a_{3}$}.
This contradicts the assumption of $a_{1}\not= a_{3}$.
%
