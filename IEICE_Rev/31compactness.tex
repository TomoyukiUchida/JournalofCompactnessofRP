...
We remark that $a_i$ and $b_j$ may be same for $i,j (1\le i,j\le 3)$.
...
\noindent
For the cases (iv), we can prove that $p \{ x := xy \} \preceq q$ holds in a similar way as Lemma \ref{two_variables}.
For the cases (v)--(x), we can prove that $p \{ x := xy \} \preceq q$ holds in a similar way as Lemma \ref{two_variables}.
