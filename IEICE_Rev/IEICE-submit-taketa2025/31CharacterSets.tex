...
\newcommand{\TheConditionAprime}{{\color{black}$b \neq a \mbox{~and~} c \neq d$}}
...
{\color{black} In this subsection, 
...
\begin{lem}\label{lem:addpart}
  ...
  {\color{black} Let {\color{black}$p$ and $q$} be regular patterns in {\color{black} $\RPat_{\Sigma\cup X}$}}.
  {\color{black} Define $D = \{ ya, bc, dy \} \subsetneq \RPat_{\Sigma\cup X}$ 
  where \TheConditionAComma~  and 
  $y$ is a variable symbol in $X$ that {\color{black}occurs in neither $p$ nor $q$}.}
  {\color{black} Suppose that $p\preceq q$ or that $q$ minimally supports $D$ for $p$. Then $p \{ x := xy \} \preceq q$}.
\end{lem}
  ...
  {\color{black} with the conditions that the symbols $a,~b,~c$ and $d$ satisfy in $D$.}
{\color{black} Since 
$q$ minimally supports $D$ for $p$, i.e., $p \{ x := r \} \preceq q$ for all $r \in D$,
the regular pattern }
$q$ can be expressed in one of the following forms: Let $y_{1}, y_{2}$ be distinct variable symbols in $X$ and $q_{1}, q_{2}, w, w^{\prime}$ either the empty string or a regular pattern {\color{black}over} $\Sigma\cup X$.
...
In (\ref{lem:addpart}-2), the substring $dya$ occurs in $q$ as a result of either variable sharing or adjacency between  $ya$ and $dy$ in D.
In (\ref{lem:addpart}-3), when $a = d$, the substring $y_1ay_2$, formed by a one-character overlap between $ya$ and $dy$ in $D$, is observed within $q$.
  ...
  \end{align*}
  {\color{black}The variables $y_{1}^{\prime}$ and $y_{2}^{\prime}$ are obtained by splitting the variables $y_{1}$ and $y_{2}$, respectively, so that regular patterns substituted for $y_{1}$ and $y_{2}$ can be divided and assigned accordingly.  
  ...
  Since $p_{1} \preceq q_{1}dy_{2}wy_{1}aw^{\prime}$  from (3), both $p_{1} \preceq q^{\prime}_{1}q^{\prime}_{2}$ and $p_{2} \preceq q^{\prime}_{2}q^{\prime}_{3}$, and $q_{2}^{\prime}$ contains a variable symbol.
  In both cases, by Theorem~\ref{Sato1:Lemma9}, {\color{black} we have} $p \preceq q$.
  {\color{black}This contradicts the assumption that $p$ and $q$ satisfy 
  $p\not\preceq q$.}
  ...
  \noindent
  \textit{Claim} 2. $(A, B, C) = (\mbox{\color{black} $dy_{2}, bc, y_{1}a$})$.
  ...
  {\color{black}This also contradicts the assumption that $p$ and $q$ satisfy 
  $p\not\preceq q$.}
  ...
  From \textit{Claim} 2, {\color{black}the} regular pattern $q$ is expressed as $q_{1}y_{1}awbcw^{\prime}dy_{2}q_{2}$, where \TheConditionA.
  ...
  From (1') and (2'), {\color{black}both }$wbcw^{\prime}d$ and $w^{\prime}d$ are prefixes of $p_{2}$, and from (2) and (3), {\color{black}both }$awbcw^{\prime}$ and $aw$ are suffixes of $p_{1}$.
  It implies a contradiction in the following inductive way:
  
  \begin{itemize}
  \item $|w|=|w^{\prime}|$: Directly, $b = d$ and $a = c$.
  \item $|w|=|w^{\prime}|+1$: Also, $a = b$.
  \item $|w| = |w^{\prime}|+2$: Since {\color{black}both }$awbcw^{\prime}$ and $aw$ are suffixes of $p_{1}$, and $|w|\geq 2$, $a$ is a suffix of $w$.
  From (1') and (2'), we have $w=w^{\prime}da$.
  Furthermore, since {\color{black}both }$awbcw^{\prime}$ and $aw$ are suffixes of $p_{1}$, it follows that $w=bcw^{\prime}$.
  ...
  \item {\color{black}$|w| < |w^{\prime}|$: The proof can be established in a manner analogous to the case where $|w|>|w^{\prime}|$.}
\end{itemize}

  From the above, we conclude that all cases of (\ref{lem:addpart}-1) contradict the assertion that {\color{black}$p\not\preceq q$} and the conditions \TheConditionA.
...
We suppose that $(A, B) = (dy_{1}a, bc)$, i.e., $q = q_{1}dy_{1}awbcq_{2}$.
  ...
  $$p=p^{\prime}_{1}p^{\prime\prime}_{1}xp_{2} \preceq {\color{black}q_{1}dp^{\prime\prime}_{1}xy_{1}^{\prime}awbcq_{2}}=q \{ y_{1}:=p^{\prime\prime}_{1}xy_{1}^{\prime} \}.$$
    {\color{black}This contradicts the assumption that $p$ and $q$ satisfy 
  $p\not\preceq q$.}
  ...
  From the above, we conclude that if $p \{ x := r \} \preceq q$ for all $r {\color{black}\in}  \{ ya, bc, dy \}$ (\TheConditionA), then $p \{ x := xy \} \preceq q$.
  \end{proof}

  \noindent
  {\color{black} Note that Lemma~\ref{lem:addpart} is valid under the condition $\sharp\Sigma \geq 2$.}
  ...
  \begin{figure*}[t]
    \begin{center}
      ...
      \caption{Let $\Sigma=\{a,b,c,d,e,f,g\}$ and {\color{black}$p,q \in \RPat_{\Sigma\cup X}$}. We assume that the symbols in $\Sigma$ are mutually distinct. The figure (3) expresses case $D = \{ ya, bc, dy \}$ in Lemma~\ref{lem:addpart}.
...
Define $D=\{ya,bc,dy\}\subsetneq \RPat_{\Sigma\cup X}$ where \TheConditionBComma, and $y$ is a variable symbol in $X$ that occurs in neither $p$ nor $q$.}
{\color{black} Suppose that $p\preceq q$ or that $q$ minimally supports $D$ for $p$.
...
The symbols occurring in $D$ correspond to either a variable or a constant symbol in $q$.
Let $y_{1}$ and $y_{2}$ be variable symbols occurring in $q$.
...
Let $A,B,C$ be regular patterns {\color{black}over} $\Sigma \cup X$, where $\{ A,B,C \} = \{ y_{1}a,ac,dy_{3} \}$.
Since $p \{ x := xy \} \not \preceq q$, $q$ can be expressed in one of the following four forms:
Let $y_{1}, y_{2}$ be distinct variable symbols in $X$, and $q_{1}, q_{2}, w, w^{\prime}$ either the empty string or a regular pattern {\color{black}over} $\Sigma\cup X$.
...
\noindent
(\ref{lem:oneside_i}-1) Case of $q=q_{1}AwBw^{\prime}Cq_{2}$, where {\color{black}$( A,B,C ) = ( y_{1}a,ac,dy_{2} )$}:
...
From (1') and (2'), {\color{black}both }$wacw^{\prime}d$ and $w^{\prime}d$ are prefixes of $p_{2}$, and from (2) and (3), {\color{black}both }$awacw^{\prime}$ and $aw$ are suffixes of $p_{1}$.
It implies a contradiction in the following inductive way:
 ...
 \item $|w^{\prime}| \ge |w|+3$: From (1') and (2'), there exists a string $w^{\prime\prime}$ of length {\color{black}$|w^{\prime}|-|w|-2$} such that $w^{\prime}=wacw^{\prime\prime}$.
  Moreover, from (1') and (2'), since $|w^{\prime}d| < |wacw^{\prime}|$ and $w^{\prime}d = wacw^{\prime\prime}d$, {\color{black} $w^{\prime\prime}d$} is a prefix of $w^{\prime}$.
  On the other hand, from {\color{black} (2) and (3)}, {\color{black} $aw$} is a suffix of $w^{\prime}$.
  Since $|w^{\prime\prime}d| + |aw| = |{\color{black} w^{\prime\prime}}| + |w| + 2 = |w^{\prime}|$, we have $w^{\prime}=w^{\prime\prime}daw$.
...
\noindent
(\ref{lem:oneside_i}-2) Case of $q=q_{1}AwBq_{2}$, where {\color{black}$( A,B ) = ( y_{1}ac,dy_{2} )$}:
...
\begin{align*}
  ...
  \textrm{(3)}~& p_{1} \preceq {\color{black}q_{1}y_{1}acw}, & \textrm{(3')}~& p_{2} \preceq q_{2}.
\end{align*}
\noindent
{\color{black} This leads to a contradiction, as demonstrated by the following inductive argument:}
\begin{itemize}
\item If $|w|=0$, from (1') and (2'), {\color{black} both $cd$ and $d$ are prefixes of $p_{2}$}. Thus, we have $c=d$.
\item If $|w|=1$, from (1') and (2'), {\color{black} both $cwd$ and $wd$ are prefixes of $p_{2}$}. Thus, we have $w=c=d$.
\item If $|w| \ge 2$, then from (1') and (2'), {\color{black}both} $cwd$ and $wd$ are prefixes of $p_{2}$. Thus, we have $cw=wd$. From Proposition~\ref{prop:repstring_base}, $c=d$.
\end{itemize}
...
\begin{align*}
  ...
  \textrm{(2)}~& p_{1} \preceq q_{1}d \mbox{~or~} p_{1} \preceq q_{1}dy_{1}^{\prime} & \textrm{(2')}~& p_{2} \preceq {\color{black} acq_{2}}, \\
...
This contradicts the assumption that $p$ and $q$ satisfy 
  $p\not\preceq q$.}
Similarly, we can show that {\color{black}the other three cases} of (2) and (1') also contradict the assumption.
...
\begin{lem}\label{lem:oneside_ii}
  ...
  Let $p$ and $q$ be} regular patterns in $\RPat_{\Sigma\cup X}$.
  {\color{black} Define} $D=\{ya,bc,dy\}\subsetneq \RPat_{\Sigma\cup X}$ where \TheConditionCComma,
...
{\color{black} The conditions in Lemmas~\ref{lem:oneside_i} and \ref{lem:oneside_ii} are illustrated in (7) and (8) in Fig.~\ref{fig:lem6bigraph}, respectively.}

\begin{figure}[t]
  \begin{center}
    \includegraphics[scale=0.525]{figs/lem6bigraph.pdf}
    \caption{Let $\Sigma=\{a,b,c,d,e,f,g\}$ and {\color{black}$p,q \in \RPat_{\Sigma\cup X}$}. We assume that the symbols in $\Sigma$ are mutually distinct.
...
\begin{prop}\label{prop:bothsides}
  {\color{black} Let $\Sigma$ be an alphabet.} 
  {\color{black} Define $D= \{ ya, bc, dy \}$, where $a,b,c,d\in \Sigma,~b=a,~c=d$ and $y$ is a variable symbol in $X$.}
  There exist regular patterns $p$ and $q$ {\color{black} in $\RPat_{\Sigma\cup X}$
  such that $q$ minimally supports $D$ for $p$ (i.e., $p\not\preceq q$, $p \{ x := r \} \preceq q$ for any $r \in D$, $p\{x:=by\}\not\preceq q$ and $p\{x:=yc\}\not\preceq q$) and $y$ occurs in neither $p$ nor $q$.}, but $p \{ x := xy \} \not \preceq q$.
\end{prop}
...
\noindent
Obviously $p \{ x:=xy \} \not \preceq q$.
For these $p$ and $q$, the condition for Proposition~\ref{prop:bothsides} holds as follows (see also {\color{black} Fig.~\ref{fig:cex-bacd}}):
\begin{eqnarray*}
&p& \{ x:=ya \} \\ 
