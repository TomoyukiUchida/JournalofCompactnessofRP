...
\begin{proof}
    ...
    It suffices to show that $S_{2}(p) \subseteq L(Q)$ implies {\color{black}$\{p\} \sqsubseteq Q$}
    for any regular pattern {\color{black}$p \in P$}.
    The proof is done by mathematical induction on $n$, where $n$ is the number of variable symbols occurs in $p$.

    In case $n=0$, $S_{2}(p) = \{p\}$.
    By (i), we have {\color{black}$\{p\} \subseteq L(Q)$}. Thus, $p \preceq q$ for some $q \in Q$.

    For $n \geq 0$,
    we assume that it is valid for any regular pattern $p$
    with $n$ variable symbols.
    Let $p$ be a regular pattern such that $n+1$ variable symbols occur in $p$
    and $S_{2}(p) \subseteq L(Q)$.
    {\color{black} Let $Q = \{q_{1},\ldots,q_{k}\}$.}
    We assume that {\color{black}$\{p\} \not\sqsubseteq Q$}, that is, {\color{black}$p \not\preceq q_{i}$}
    for any $i \in \{1,\ldots,k\}$.
    {\color{black} Let $p_{1}$, $p_{2}$ be regular patterns and $x$ a variable symbol with $p = p_{1}xp_{2}$.}
    For $a, b \in \Sigma$,
    let $p_{a}=p\{x:=a\}$ and $p_{ab}=p\{x:=ab\}$.
    Both $p_{a}$ and $p_{ab}$ have $n$ variable symbols, respectively.
    Thus, $S_{2}(p_{a}) \subseteq L(Q)$ and $S_{2}(p_{ab}) \subseteq L(P)$.
    ...
    Since $k \geq 3$, {\color{black} $2k-1 \geq k+2$}.
...
{\color{black}the following corollary holds}.


\begin{col}\label{Col18}
    %Let $k \geq 3$, $\sharp\Sigma \geq 2k-1$ and $P \in \RPatplus$.
    {\color{black}Let $k$ be an integer with $k \geq 3$ and $\Sigma$ an alphabet with $\sharp\Sigma \geq 2k-1$.
    Then, for a set of regular patterns $P \in \RPatplus_{\Sigma\cup X}$,}
    $S_{2}(P)$ is a characteristic set for $L(P)$ within $\RPatLkei_{\Sigma\cup X}$.
\end{col}

{\color{black}From Theorem~\ref{Theorem17},
we have the following theorem.}

\begin{thm}\label{Theorem5}
    %Let $k \geq 3$ and $\sharp\Sigma \geq 2k-1$.
    {\color{black}Let $k$ be an integer with $k \geq 3$ and $\Sigma$ an alphabet with $\sharp\Sigma \geq 2k-1$.}
    Then, {\color{black}$\RPatkei_{\Sigma\cup X}$} has compactness with respect to {\color{black} language} containment.
\end{thm}
...
\begin{lem}[Sato et al.\cite{Sato1}]\label{Sato1:Lemma19}
    %Let $k \geq 3$ and $\sharp\Sigma \leq 2k-2$.
    {\color{black} Let $k$ be an integer with $k \geq 3$ and $\Sigma$ an alphabet with $\sharp\Sigma \leq 2k-2$.}
    Then, {\color{black}$\RPatkei_{\Sigma\cup X}$} does not have compactness with respect to {\color{black} language} containment.
\end{lem}

\begin{proof}
    Let $\Sigma =\{a_{1},\ldots,a_{k-1},b_{1},\ldots,b_{k-1}\}${\color{black},
    ...
    since $p \not \preceq q_{i}$, $L(p) \not \subseteq L(q_{i})$
    for any $i \in \{1,\ldots, k\}$.
    Therefore, $\RPatkei$ does not have compactness with respect to {\color{black} language} containment.
\end{proof}

In case $k=2$, we have the following theorem.

\begin{thm}\label{Theorem21}
    {\color{black} Let $\Sigma$ be an alphabet with $\sharp\Sigma \geq 4$.
    For two sets of regular patterns $P \in \RPatplus_{\Sigma\cup X}$ and $Q \in \RPat^{2}_{\Sigma\cup X}$,
...
\begin{proof}
    ...
    where $n$ is the number of variable symbols occurring in $p$.
...
{\color{black} The following example provides a set of regular patterns $P \in \RPat^{+}$ and a set of regular patterns $Q \in \RPat^{2}$ demonstrating that, when $\sharp\Sigma = 3$, the three conditions (i), (ii), and (iii) stated in Theorem~\ref{Theorem21} are not equivalent.}
...
\begin{ex}
    ...
    Let $\Sigma = \{a,b,c\}$.
    Define the regular patterns $p = x'axbx''$, $q_{1} = x'abx''$, and $q_{2} = x'cx''$,
    where $x,x',x''$ are variable symbols in $X$.
    Then the following statements hold:
    \begin{itemize}
    \item[(1)] For any constant string $w \in \Sigma^{+}$, if $w$ contains the symbol $c$, then $p \{x:=w\} \preceq q_{2}$.
    \item[(2)]  For any constant string $w \in \Sigma^{+}$, if $w$ does not contain $c$, then  $p\{x:=w\} \preceq q_{1}$.
    \end{itemize}
    Consequently, $L(p) \subseteq L(q_{1}) \cup L(q_{2})$.
    However, $p \not\preceq q_{1}$ and $p \not\preceq q_{2}$.}
\end{ex}
...
\begin{col}
    {\color{black} Let $\Sigma$ be an alphabet with $\sharp\Sigma \geq 4$.
    For a set of regular patterns $P \in \RPatplus_{\Sigma\cup X}$,}
    $S_{2}(P)$ is a characteristic set for $L(P)$ within {\color{black}$\RPatL^{2}_{\Sigma\cup X}$}.
\end{col}

\begin{col}
    {\color{black} Let $\Sigma$ be an alphabet with $\sharp\Sigma \geq 4$}.
Then, {\color{black}$\RPat^{2}_{\Sigma\cup X}$} has compactness with respect to {\color{black} language} containment.
\end{col}
