...
Let $\blacktriangle$ be a constant symbol that is not a member in $\Sigma$.
We define the functions $\sigma_{A}: Q \rightarrow \Sigma \cup \{\blacktriangle\}$ and $\sigma_{B}: Q \rightarrow \Sigma \cup \{\blacktriangle\}$ as follows:
\begin{align*}
  \sigma_{A}(q) & =
  \begin{cases}
    a & \textrm{if } A(q) = \{a\}, \\
    \blacktriangle & \textrm{if } A(q) = \emptyset.
  \end{cases}\\
  \sigma_{B}(q) & =
  \begin{cases}
    b & \textrm{if } B(q) = \{b\}, \\
    \blacktriangle & \textrm{if } B(q) = \emptyset.
  \end{cases}
\end{align*}
The inverse functions of $\sigma_{A}$ and $\sigma_{B}$ are denoted by $\sigma_{A}^{-1}$ and $\sigma_{B}^{-1}$, respectively. That is, for $a,b \in \Sigma \cup \{\blacktriangle\}$, let $\sigma_{A}^{-1}(a) = \{q \in Q \mid \sigma_{A}(q) = a\}$ and $\sigma_{B}^{-1}(b) = \{q \in Q \mid \sigma_{B}(q) = b\}$. 
We give an example in Fig.~\ref{fig:lem8eachreg}.

$A$ and $B$ denotes the following subsets of $\Sigma$:
\begin{align*}
  A & = \bigcup_{q \in Q \setminus \sigma_{A}^{-1}(\blacktriangle)} A(q) \mbox{,~~~}
  B = \bigcup_{q \in Q \setminus \sigma_{B}^{-1}(\blacktriangle)} B(q).
\end{align*}
...
\begin{enumerate}
  ...
  \item[(ii)] $\sharp A + \ell_{A} + \sharp \sigma_{A}^{-1}(\blacktriangle) = \sharp B + \ell_{B} + \sharp \sigma_{B}^{-1}(\blacktriangle) = k$.
\end{enumerate}
...
\begin{align*}
  %Q^{(\blacktriangle,\blacktriangle)} & = \{q \in Q \mid \sigma_{A}(q) = \blacktriangle \textrm{~and~} \sigma_{B}(q) = \blacktriangle\},\\
  Q^{(\blacktriangle,\blacktriangle)} & = \sigma_{A}^{-1}(\blacktriangle) \cap \sigma_{B}^{-1}(\blacktriangle),\\
  %Q^{(\blacktriangle,\cdot)} & = \{q \in Q \mid \sigma_{A}(q) = \blacktriangle \textrm{~and~} \sigma_{B}(q) \not= \blacktriangle\},\\
  Q^{(\blacktriangle,\cdot)} & = \sigma_{A}^{-1}(\blacktriangle) \cap (Q\setminus \sigma_{B}^{-1}(\blacktriangle)),\\
  %Q^{(\cdot,\blacktriangle)} & = \{q \in Q \mid \sigma_{A}(q) \not= \blacktriangle \textrm{~and~} \sigma_{B}(q) = \blacktriangle\},\\
  Q^{(\cdot,\blacktriangle)} & = (Q\setminus \sigma_{A}^{-1}(\blacktriangle)) \cap \sigma_{B}^{-1}(\blacktriangle),\\
  %Q^{(\cdot,\cdot)} & = \{q \in Q \mid \sigma_{A}(q) \not= \blacktriangle \textrm{~and~} \sigma_{B}(q) \not= \blacktriangle\}.
  Q^{(\cdot,\cdot)} & = (Q\setminus \sigma_{A}^{-1}(\blacktriangle)) \cap (Q\setminus \sigma_{B}^{-1}(\blacktriangle)).
\end{align*}
...
It is easy to see that if $w \in (A\cdot B) \cup (A'\cdot B) \cup (A\cdot B')$, there exists a regular pattern $q_{w} \in Q^{(\blacktriangle,\cdot)} \cup Q^{(\cdot,\blacktriangle)} \cup Q^{(\cdot,\cdot)}$ such that $p \{ x:=w \} \preceq q_{w}$ holds.
...
\noindent
\textit{Claim} 2. If there exist $q \in Q^{(\blacktriangle,\blacktriangle)}$ and distinct $5$ strings $w_{i} \in A'\cdot B'$ ($1\leq i\leq 5$) such  that $p \{ x:=w_{i} \} \preceq q$ holds ($1\leq i\leq 5$),  then $p \{ x:=xy \} \preceq q$ holds.

\smallskip

\noindent
\textit{Claim} 3. If there exist $q \in Q^{(\blacktriangle,\cdot)} \cup Q^{(\cdot,\blacktriangle)}$ and distinct $3$ strings $w_{i} \in A'\cdot B'$ ($1\leq i\leq 3$) such that $p \{ x:=w_{i} \} \preceq q$ holds ($1\leq i\leq 3$),  then $p \{ x:=xy \} \preceq q$ holds.

\smallskip

\noindent
If there exist a regular pattern $q \in Q^{(\blacktriangle,\blacktriangle)} \cup Q^{(\blacktriangle,\cdot)} \cup Q^{(\cdot,\blacktriangle)}$ and enough strings $w \in A'\cdot B'$ such that either of the conditions of \textit{Claims} 2 and 3 is satisfied, this lemma holds. Then, we assume that it is not the case.

\smallskip

\noindent
\textit{Assumption} 1.
There is no regular pattern $q \in Q^{(\blacktriangle,\blacktriangle)}$ and $5$ strings $w \in A'\cdot B'$ such that the condition of \textit{Claim} 2 is satisfied and there is no regular pattern $q \in Q^{(\blacktriangle,\cdot)} \cup Q^{(\cdot,\blacktriangle)}$ and $3$ strings $w \in A'\cdot B'$ such that the condition of \textit{Claim} 3 is satisfied.

\smallskip

\noindent
Let ${\cal L}_{1} = \sharp\{w \in A'\cdot B' \mid \exists q \in Q^{(\blacktriangle,\blacktriangle)} \cup Q^{(\blacktriangle,\cdot)} \cup Q^{(\cdot,\blacktriangle)} \mbox{ s.t. } p\{x:=w\} \preceq q\}$.
Under \textit{Assumption} 1, each $q\in Q^{(\blacktriangle,\blacktriangle)}$ has at most $4$ strings $w \in A'\cdot B'$ such that the condition of \textit{Claim} 2 is satisfied, and each $q \in Q^{(\blacktriangle,\cdot)} \cup Q^{(\cdot,\blacktriangle)}$ has at most $2$ strings $w \in A'\cdot B'$ such that the condition of \textit{Claim} 3 is satisfied.
Then, by \textit{Claim} 1,
\begin{align*}
  {\cal L}_{1} &\leq 4\sharp Q^{(\blacktriangle,\blacktriangle)} + 2\sharp Q^{(\blacktriangle,\cdot)} + 2\sharp Q^{(\cdot,\blacktriangle)}\\
  & = 2(\sharp Q^{(\blacktriangle,\blacktriangle)} + \sharp Q^{(\blacktriangle,\cdot)}) + 2(\sharp Q^{(\blacktriangle,\blacktriangle)} + \sharp Q^{(\cdot,\blacktriangle)})\\
  & = 2\sharp \sigma_{A}^{-1}(\blacktriangle) + 2\sharp \sigma_{B}^{-1}(\blacktriangle)\\
...
From \textit{Claim} 1, $\ell_B + 2 = k - \sharp\sigma^{-1}_{B}(\blacktriangle) - \sharp B = \sharp B' - \sharp\sigma^{-1}_{B}(\blacktriangle)$ holds. Therefore, ${\cal L}_{2} \leq \ell_{B} + 2$ holds.
Thus,
\begin{align*}
  &\ \sharp A' \times \sharp B' - {\cal L}_{1} - {\cal L}_{2}\\
