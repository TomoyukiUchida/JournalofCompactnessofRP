...
\begin{lem}\label{lem:addpart}
  Let $\Sigma$ be an alphabet with $\sharp\Sigma \ge 3$ and {\color{red}$p,q$} regular patterns in {\color{red} $\RPat_{\Sigma\cup X}$}.
  ...
  Then, {\color{red} if $q$ minimally supports $D$ for $p$, then $p \{ x := xy \} \preceq q$ holds}.
\end{lem}
...
$D=\{ya,bc,dy\}\subsetneq \RPat_{\Sigma\cup X}$ where \TheConditionBComma, and $y$ is a variable symbol in $X$ that appears in neither $p$ nor $q$.}
Then, if {\color{red} $q$ minimally supports $D$ for $p$},
...
\begin{lem}\label{lem:oneside_ii}
  {\color{red}Let $\Sigma$ be an alphabet with $\sharp\Sigma \ge 3$, $p,q$ regular patterns in $\RPat_{\Sigma\cup X}$ and
  $D=\{ya,bc,dy\}\subsetneq \RPat_{\Sigma\cup X}$ where \TheConditionCComma,
  and $y$ is a variable symbol in $X$ that appears in neither $p$ nor $q$.}
  Then, {\color{red} if $q$ minimally supports $D$ for $p$,
