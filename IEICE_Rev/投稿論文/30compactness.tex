...
Therefore, in the following lemmas and propositions, let $p=p_{1}xp_{2}$, where {\color{red}$p_{i}\in \RPat_{\Sigma\cup X}$} ($i=1,2$) and $x$ is a variable symbol.
...
{\color{red} In the following two examples, we have $p\{x:=r\}\preceq q$ for $r\in D$}.
...
{\color{red} Hence, $p\{x:=xy\}\preceq q$.}
...
Let $p$ and $q$ be regular patterns in {\color{red}$\RPat_{\Sigma\cup X}$}.
