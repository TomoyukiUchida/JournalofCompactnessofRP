...
%the set $D$ is said to be \textit{maximally generalized {\color{red}with respect to} ($p,q$)}.
{\color{red}The pattern $q$ is said to \textit{minimally support $D$ for $p$}, in the sense that any generalization of the strings in $D$ would invalidate the condition $p\{x := r\} \preceq q$ for all $r\in D$}.

\end{dfn}

%\noindent{\color{red} In Subsecs.~\ref{subsec:d3a}--\ref{subsec:d3c}, we consider a subset $D \subset \RPat$ such that for $p,q \in \RPat$, 
%$D$ is maximally generalized with respect to $(p,q)$.}
...
%$p,q$ regular patterns on $\Sigma\cup X$.
