...
\end{lem}
  ...
  \item[(II)] $q=q_{1}AwBq_{2}$, where $\{ A,B \} = \{ dy_{1}a,bc \}$ for a variable symbol $y_{1}\in X$ and $w$ is either an empty string or a regular pattern on $\Sigma\cup X$.
  \item[(III)] $q=q_{1}AwBq_{2}$, where $\{ A,B \} = \{ y_{1}ay_{2},bc \}$ ($a = d$) for distinct variable symbols $y_{1}, y_{2}\in X$, and $w$ is either an empty string or a regular pattern on $\Sigma\cup X$.
  \end{enumerate}
  
  Firstly, we will prove that for the case (I), $p \{ x := xy \} \preceq q$ holds.
...
\end{figure}
  ...
  Secondly, we will prove that for the case (II), $p \{ x := xy \} \preceq q$ holds. Suppose that $(A, B) = (dy_{1}a, bc)$, i.e., $q = q_{1}dy_{1}awbcq_{2} $ holds. Then, the following conditions hold: for $y_{1}^{\prime}\in X$,
  \begin{align*}
  ...
  Finally, we will prove that for the case (III), $p \{ x := xy \} \preceq q$ holds. Suppose that $(A, B) = (y_{1}ay_{2}, bc)$, i.e., $q = q_{1}y_{1}ay_{2}wbcq_{2} $ holds. Then, the following conditions hold: for $y_{1}^{\prime}\in X$,
  \begin{align*}
