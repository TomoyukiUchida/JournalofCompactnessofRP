...
for a subset $D=\{ya,bc,dy\} \subset \RPat_{\Sigma\cup X}$,
...
\begin{lem}\label{lem:addpart}
  ...
  {\color{red} if $q$ minimally supports $D$ for $p$, then $p \{ x := xy \} \preceq q$ {\color{red} holds}}.
\end{lem}
...
\begin{lem}\label{lem:oneside_ii}
  Let $\Sigma$ be an alphabet with $\sharp\Sigma \ge 3$ and let $p,~q$ be regular patterns {\color{red}over} $\Sigma\cup X$.
  {\color{red} Let $D=\{ya,bc,dy\}\subset \RPat_{\Sigma\cup X}$ where \underline{\TheConditionCComma},
