...
%{\color{red} In this subsection, if $D = \{ ya, bc, dy \} \subset \RPat_{\Sigma\cup X}$ satisfies $a=c$ or $b=d$, then Lemmas \ref{lem:addpart}, \ref{lem:oneside_i} and \ref{lem:oneside_ii} are trivially reducible to Lemma \ref{lem:twovariables}.
% Then, in Lemmas \ref{lem:addpart}, \ref{lem:oneside_i} and \ref{lem:oneside_ii}, we consider the case where $a\neq c$ and $b\neq d$.
 % and $D$ is not maximally generalized with respect to $(p,q)$.
% }
...
%for any two regular patterns $p,q$ over $\Sigma\cup X$ with $p\not\preceq q$, we consider $D=\{ya,bc,dy\}\subset \RPat_{\Sigma\cup X}$ such that $q$ minimally supports $D$ for $p$.
Obviously, we remark that $a\neq c$ and $b\neq d$ hold,
since $q$ minimally supports $D$ for $p$.
%Hence, the conditions associated with the symbols a, b, c, and d in D are considered in Lemmas 5 through 7.
}


\begin{lem}\label{lem:addpart}
  Let $\Sigma$ be an alphabet with $\sharp\Sigma \ge 3$ and $p,~q$ regular patterns in {\color{red} $\RPat_{\Sigma\cup X}$ with $p\not\preceq q$}.
  {\color{red} Let $D = \{ ya, bc, dy \} \subset \RPat_{\Sigma\cup X}$ 
  %such that $D$ is maximally generalized {\color{red}with respect to} ($p,q$), where \TheConditionAprime~(that is, \TheConditionA),
  where \underline{\TheConditionAComma}~  and 
  ...
  %\end{enumerate}
  Then, 
  %{\color{red} if for all $r \in D$, $p \{ x := r \} \preceq q$ holds,} then $p \{ x := xy \} \preceq q$ {\color{red} holds}.
  {\color{red} if $q$ minimally supports $D$ for $p$, then $p \{ x := xy \} \preceq q$ holds}.
\end{lem}
...
%
%Since $D$ is maximally generalized {\color{red}over} $(p,q)$, 
{\color{red} Since 
%$p\{x:=r\} \preceq q$ holds for all $r\in D$,
$q$ minimally supported $D$ for $p$, i.e., $p \{ x := r \} \preceq q$ for all $r \in D$,
the regular pattern }
$q$ can be expressed in one of the following forms: Let $y_{1}, y_{2}$ be distinct variable symbols in $X$ and $q_{1}, q_{2}, w, w^{\prime}$ be either the empty string or a regular pattern {\color{red}over} $\Sigma\cup X$.
...
\smallskip
  ...
  \noindent
  %{\color{red} Here, we remark that 
  %each string in $D$ occurs independently within $q$ in (\ref{lem:addpart}-1),
  %the string $dya$ is observed as a result of either variable sharing or adjacency between $ya$ and $dy$ in $D$ in (\ref{lem:addpart}-2),
  %in the case where $a=d$, the string $y_1ay_2$, which is formed by a one-character overlap between $ya$ and $dy$ in $D$, occurs within $q$ in (\ref{lem:addpart}-3).}
  {\color{red}
...
%    & p_{1} \preceq q_{1}y_{1}^{\prime}, & & \\
  ...
  \noindent
  {\color{red} Note that Lemma~\ref{lem:addpart} is valid under the condition that $\sharp\Sigma \geq 2$.}
  ...
  \begin{figure*}[t]
    \begin{center}
      ...
      \caption{Let $\Sigma=\{a,b,c,d,e,f,g\}$ and {\color{red}$p,q \in \RPat_{\Sigma\cup X}$ with $p\not\preceq q$}. We assume that the symbols in $\Sigma$ are mutually distinct. The figure (3) expresses case $D = \{ ya, bc, dy \}$ in Lemma~\ref{lem:addpart}.
      ...
      % and $D$ is maximally generalized {\color{red}with respect to} ($p,q$), 
      then $p \{ x := xy \} \preceq q$ holds.}\label{fig:lem5bigraph}
    \end{center}
  \end{figure*}


\begin{lem}\label{lem:oneside_i}
Let $\Sigma$ be an alphabet with $\sharp\Sigma \ge 3$, $p,~q$ regular patterns {\color{red} in $\RPat_{\Sigma\cup X}$ with $p\not\preceq q$ and
$D=\{ya,bc,dy\}\subsetneq \RPat_{\Sigma\cup X}$ where \underline{\TheConditionBComma}, and $y$ is a variable symbol in $X$ that does not appear in $p$ and $q$.}
...
%\end{enumerate}
Then, if 
%{\color{red} for all $r \in D$,} $p \{ x := r \} \preceq q$ {\color{red} holds},
{\color{red} $q$ minimally supports $D$ for $p$},
 then $p \{ x := xy \} \preceq q$ {\color{red} holds}.
\end{lem}
...
%Since $p \{ x := r \} \preceq q$ for all $r \in D$ and $D$ is maximally generalized {\color{red}with respect to} $(p,q)$, 
...
For $p_{1} \preceq q_{1}d$ in (1) and $p_{2} \preceq acq_{2}$ in (3'), $p = p_{1}xp_{2} \preceq q_{1}dxacq_{2} \preceq q\{y_{1}:=x\}$ holds. 
%From this, we have $p\{x:=xy\} \preceq q\{y_{1}:=x\}$. 
{\color{red}
From this, we have $p\preceq q$.
This contradicts the assumption that $p$ and $q$ satisfy 
  $p\not\preceq q$ holds.}
%This contradicts the assumption that $p\{x:=xy\} \not\preceq q$. 
Similarly, we can show that the other cases of (1) and (3') also contradict the assumption.
...
\begin{lem}\label{lem:oneside_ii}
  {\color{red}Let $\Sigma$ be an alphabet with $\sharp\Sigma \ge 3$, $p,~q$ be regular patterns in $\RPat_{\Sigma\cup X}$ with $p\not\preceq q$ and
  $D=\{ya,bc,dy\}\subsetneq \RPat_{\Sigma\cup X}$ where \underline{\TheConditionCComma},
  ...
  %\end{enumerate}
  Then, 
  %{\color{red}if for all $r \in D$, $p \{ x := r \} \preceq q$ holds,
  {\color{red} if $q$ minimally supports $D$ for $p$,
...
\begin{figure}[t]
  \begin{center}
    ...
    \caption{Let $\Sigma=\{a,b,c,d,e,f,g\}$ and {\color{red}$p,q \in \RPat_{\Sigma\cup X}$ with $p\not\preceq q$}. We assume that the symbols in $\Sigma$ are mutually distinct.
    ...
    In these cases, if $p \{ x := r \} \preceq q$ for all $r \in D$,
    % and $D$ is maximally generalized {\color{red}with respect to} ($p,q$),
     then $p \{ x := xy \} \preceq q$ holds.}\label{fig:lem6bigraph}
  \end{center}
\end{figure}
