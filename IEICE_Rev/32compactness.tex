...
% }
{\color{red} In this subsection, for any two regular patterns $p,q$ over $\Sigma\cup X$ with $p\not\preceq q$, we consider $D=\{ya,bc,dy\}\subset \RPat_{\Sigma\cup X}$ such that $D$ is maximally generalized with respect to $(p,q)$.
The conditions associated with the symbols a, b, c, and d in D are considered in Lemmas 5 through 7.
Obviously, we remark that $D$ satisfies $a\neq c$ and $b\neq d$.}
...
\end{lem}
 ...
 %We assume that $p \{ x := xy \} \not\preceq q$ in order to derive a contradiction.
  We assume that 
  $p \{ x := xy \} \not\preceq q$
  %{\color{red} $D$ is maximally generalized with respect to $(p,q)$}
...
{\color{red} Since $p\{x:=r\} \preceq q$ holds for all $r\in D$,}
the regular pattern $q$ can be expressed in one of the following forms: Let $y_{1}, y_{2}$ be distinct variable symbols in $X$ and $q_{1}, q_{2}, w, w^{\prime}$ be either the empty string or a regular pattern {\color{red}over} $\Sigma\cup X$.
...
%\end{enumerate}
Then, if {\color{red} for all $r \in D$,} $p \{ x := r \} \preceq q$ {\color{red} holds}, then $p \{ x := xy \} \preceq q$ {\color{red} holds}.
\end{lem}
...
\begin{lem}\label{lem:oneside_ii}
  ...
  %\end{enumerate}
  Then, {\color{red}if for all $r \in D$, $p \{ x := r \} \preceq q$ holds, then $p \{ x := xy \} \preceq q$ holds}.
\end{lem}
