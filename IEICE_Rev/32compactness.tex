...
When either $|w|=|w^{\prime}|$ or $|w|=|w^{\prime}|+1$, Eq.~(\ref{prop:repstring_eq}) is depicted as shown in Figs.~\ref{追加部分7} and \ref{追加部分8}, respectively. It When $|w|=|w^{\prime}|$, $(b, c) = (d, a)$ holds. When $|w|=|w^{\prime}|+1$, $a$ is a prefix of $w$ and $b$ is a suffix of $w$. Let $u$ be a string such that $aub=w$. Let $u^{\prime}=w^{\prime}$. Since $wdaw^{\prime} = w^{\prime}bcw$, we have $aubdau^{\prime} = u^{\prime}bcaub$.

Trivially, these cases contradict the conditions $b \not\in \{a,d\}$ and $c \not\in \{a,d\}$.
