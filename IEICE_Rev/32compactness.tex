...
We note that the conclusion of Proposition~\ref{prop:repstring_base} is the same as $(a, d) = (b, c)$ or $(a, d) = (c, b)$. Thus, if $daw = wbd$, we get the same conclusion. 
...
\end{align}
holds, then $(b, c) = (a, d)$ or $(b, c) = (d, a)$ hold.\label{prop:repstring_eq}
\end{prop}

\begin{proof}
We will prove this proposition by an induction on $|w| + |w^{\prime}|$.
W.l.o.g., we suppose that $|w| \geq |w^{\prime}|$ holds.
If $|w| > |w^{\prime}$ holds, we get $(a, d) = (b, c)$ or $(a, d) = (c, b)$ as its conclusion. It is the same as the conclusion of this Proposition.
\begin{enumerate}
  \item[(i)] $|w| \geq 0$ and $|w^{\prime}|=0$:
  Eq.~(\ref{prop:repstring_eq}) becomes $wda = bcw$. From Proposition~\ref{prop:repstring_base}, $(b, c) = (a, d)$ or $(b, c) = (d, a)$ hold.
\end{enumerate}
We assume that for constant strings $u$ and $u^{\prime}$ with $|u| + |u^{\prime}| < |w| + |w^{\prime}|$, if $udau^{\prime} = u^{\prime}bcu$ holds, then $(b, c) = (a, d)$ or $(b, c) = (d, a)$ hold.
...
It When $|w|=|w^{\prime}|$, $(b, c) = (d, a)$ holds. When $|w|=|w^{\prime}|+1$, $a = c$ and $w = w^{\prime}b = dw^{\prime}$ hold.
From Proposition~\ref{prop:repstring_orig}, we get $b = d$.
Therefore, $(b, c) = (a, d)$ or $(b, c) = (d, a)$ hold.
%
...
Since $|u| = |w| - |w^{\prime}| - 2 \leq |w| - 3 < |w|$ and $|u^{\prime}| = 2|w^{\prime}| - |w| < |w|$, i.e., $|u| + |u^{\prime}| < |w| + |w^{\prime}|$ holds, from the induction hypothesis on $|u| + |u^{\prime}|$, $(b, c) = (a, d)$ or $(b, c) = (d, a)$ hold.
%
\item[(iv)] $2|w^{\prime}| \le |w| \le 2|w^{\prime}|+3$:
When $|w|=2|w^{\prime}|$, we easily see that $w = w^{\prime}w^{\prime}$.
Therefore, $w^{\prime}da = bcw^{\prime}$ holds as shown in Fig.~\ref{追加部分14}. From Proposition~\ref{prop:repstring_base}, $(b, c) = (a, d)$ or $(b, c) = (d, a)$ hold.
When $|w|=2|w^{\prime}|+i$ ($i=1,2,3$), Eq.~(\ref{prop:repstring_eq}) is depicted as shown in Figs.~\ref{追加部分13}, \ref{追加部分12}, and \ref{追加部分11}.
When $|w|=2|w^{\prime}|+2$, trivially we have $(b, c) = (d, a)$. 
When $|w|=2|w^{\prime}|+1$ and $|w|=2|w^{\prime}|+3$, from Proposition~\ref{prop:repstring_orig}, $(b, c) = (a, d)$ holds.
%
...
Let $u^{\prime} = w^{\prime}$.
Since $|u| + |u^{\prime}| = |w|- (2|w^{\prime}| + 4) + |w^{\prime}| < |w| + |w^{\prime}|$, from the induction hypothesis on $|u| + |u^{\prime}|$, $(b, c) = (a, d)$ or $(b, c) = (d, a)$ hold.
\end{enumerate}
From the above, we conclude that if $wdaw^{\prime} = w^{\prime}bcw$, $(b, c) = (a, d)$ or $(b, c) = (d, a)$ hold.
\end{proof}
...
\end{lem}
  ...
  From Proposition~\ref{prop:repstring_base}, $(b, c) = (a, d)$ or $(b, c) = (d, a)$ hold.
  Therefore, these cases contradict the conditions $b \not\in \{a,d\}$ and $c \not\in \{a, d\}$.
  ...
  From Proposition~\ref{prop:repstring}, we have $(b, c) = (a, d)$ or $(b, c) = (d, a)$.
  It contradicts the conditions $b \not\in \{a,d\}$ and $c \not\in \{a, d\}$.
