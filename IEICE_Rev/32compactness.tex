...
\end{align}
holds, $a = b$ and $c = d$ hold.
\end{prop}

\begin{proof}
For $i=1,\ldots,|w|$, we refer the $i$-th symbol of $w$ as $w[i - 1]$.
\begin{enumerate}
\item If $|w| = 0$, eq.\ref{prop:repstring_base_eq} becomes $da = bc$. Thus, $a = b$ and $c = d$ hold.
\item If $|w| = 1$, from $w[0]da = bcw[0]$, we have $w[0] = b$, $d = c$, and $a = w[0]$. Thus, $a = b$ and $c = d$ hold.
\item If $|w| = 2$, from $w[0]w[1]da = bcw[0]w[1]$, we have $w[0] = b$, $w[1] = c$, $d = w[0]$, $a = w[1]$. Thus, $a = b$ and $c = d$ hold.
\item If $|w| = 3$, from $w[0]w[1]w[2]da = bcw[0]w[1]w[2]$, we have $w[0] = b$, $w[1] = c$, $w[2] = w[0]$, $d = w[1]$, $a = w[2]$. Thus, $a = b$ and $c = d$ hold.
\item Let $n = |w|$. When $n \ge 4$, we assume that for any string $w^{\prime}$ with $|w^{\prime}|<n$, if $w^{\prime}da = bcw^{\prime}$ holds, $a = b$ and $c = d$ hold.
Since the string $w$ has a prefix $bc$ and a suffix $da$, there exists a string $w^{\prime}$ with $0 \leq |w^{\prime}| \leq n - 4$ such that $w = bcw^{\prime}da$ holds.
Since $wda = bcw^{\prime}dada$ and $bcw = bcbcw^{\prime}da$, if $wda = bcw$ holds, we have $w^{\prime}da = bcw^{\prime}$. Thus, from the assumption, we get $a = b$ and $c = d$.
\end{enumerate}
\end{proof}
...
\end{lem}
  ...
  If $|w|=|w^{\prime}|$, then $b = d$ and $a = c$ hold.
%  It contradicts the conditions $b \not\in \{a,d\}$ and $c \not\in \{a, d\}$.
  If $|w|=|w^{\prime}|+1$, then $a = b$ holds.
%  It contradicts the condition $b \not = a$.
  ...
  From Proposition~\ref{prop:repstring_base}, $a = b$ and $c = d$ hold. Threfore, these cases contradict the conditions $b \not\in \{a,d\}$ and $c \not\in \{a, d\}$.
