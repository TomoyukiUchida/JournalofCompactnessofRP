...
\end{lem}
  ...
  Let $p=p_{1}xp_{2}$, where $p_{1}, p_{2}$ are regular patterns on $\Sigma\cup X$ and $x$ is a variable symbol in $X$.
  We assume that $p \{ x := xy \} \not \preceq q$ in order to derive the contradiction.

  Since $p \{ x := xy \} \not \preceq q$, there exist three regular patterns $A,B,C$ on $\Sigma\cup X$ such that $q=q_{1}AwBw^{\prime}Cq_{2}$ hold, where $\{ A,B,C \} = \{ y_{1}a,bc,dy_{2} \}$ for some variable symbols $y_{1}, y_{2}$ ($y_{1} \not= y_{2}$), and each of $w, w^{\prime}$ is an empty string or a regular pattern on $\Sigma\cup X$.
  Firstly, we show the following claim:
