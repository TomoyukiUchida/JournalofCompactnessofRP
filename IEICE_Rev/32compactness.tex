...
\end{itemize}
We assume that for constant strings $u$ and $u^{\prime}$ with $|u| + |u^{\prime}| < |w| + |w^{\prime}|$, if $udau^{\prime} = u^{\prime}bcu$ holds, then $\pair{b}{c} \in \{\pair{a}{d}, \pair{d}{a}\}$ holdss.
...
\end{itemize}
  ...
  \begin{figure*}[t]
    \begin{center}
      ...
      \caption{Let $\Sigma=\{a,b,c,d,e,f,g\}$ and $p,q \in \RPat$. We assume that the symbols in $\Sigma$ are mutually distinct. The figure (3) expresses the condition of Lemma~\ref{lem:addpart} for the case $D = \{ ya, bc, dy \}$. This is the most general case.
      The figures (4), (5), and (6) express the conditions of of Lemma~\ref{lem:addpart} in the cases that (4) $D = \{ ya, bc, ay \}$, (5) $D = \{ ya, bb, dy \}$, and (6) $D = \{ ya, bb, ay \}$, respectively. In these cases, if $p \{ x := r \} \preceq q$ for all $r \in D$, then $p \{ x := xy \} \preceq q$ holds.}\label{fig:lem5bigraph}
    \end{center}
  \end{figure*}
...
The conditions (i) and (ii) in Lemma~\ref{lem:oneside} are illustrated in the cases (7) and (8) in Fig.~\ref{fig:lem6bigraph}, which are additional cases to the cases (3)--(6) in Fig.~\ref{fig:lem5bigraph}.
...
\begin{figure}[t]
  \begin{center}
    ...
    The figures (7) and (8) express the conditions of of Lemma~\ref{lem:oneside} in the cases that (7) $D = \{ ya, ac, dy \}$ and (8) $D = \{ ya, bd, dy \}$, respectively. The condition in Lemma~\ref{lem:oneside} also include the cases (3)--(6) in Fig.~\ref{fig:lem5bigraph}.  In these cases, if $p \{ x := r \} \preceq q$ for all $r \in D$, then $p \{ x := xy \} \preceq q$ holds.}\label{fig:lem6bigraph}
  \end{center}
\end{figure}
