...
\end{lem}
  ...
  First of all, we show that the cases (b) and (c) are not possible. Suppose that there exists $y_{3}c$ in $q$ that corresponds to $bc$ in $p\{x:=bc\}$. The regular pattern $q$ can be expressed in one of the following forms (b-1)--(b-5): Let $y_{1}, y_{2}, y_{3}$ be distinct variable symbols in $X$ and $w,w^{\prime}$ either an empty string or a regular pattern on $\Sigma\cup X$.
  \begin{enumerate}
    \item[(b-1)] $q=q_{1}AwBw^{\prime}Cq_{2}$, where $\{ A,B,C \} = \{ y_{1}a, dy_{2}, y_{3}c \}$.
    \item[(b-2)] $q=q_{1}AwBq_{2}$, where $\{ A,B \} = \{ dy_{1}a, y_{3}c \}$.
    \item[(b-3)] $q=q_{1}AwBq_{2}$, where $\{ A,B \} = \{ dy_{2}c, y_{1}a \}$.
    \item[(b-4)] $q=q_{1}AwBq_{2}$, where $\{ A,B \} = \{ y_{1}ay_{2}, y_{3}c \}$ ($a = d$).
    \item[(b-5)] $q=q_{1}Aq_{2}$, where $A = y_{1}ay_{2}c$ ($a = d$).
  \end{enumerate}
...
\end{align*}
  ...
  Additionally, contradictions are derived from Theorem 2 in cases (b-2)--(b-5) and also in all cases of (c).
