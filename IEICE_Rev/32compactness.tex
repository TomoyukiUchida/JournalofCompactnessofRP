...
\end{lem}
  ...
  %
  %The regular pattern $q$ can be expressed in one of the following forms (b1)--(b5): Let $y_{1}, y_{2}, y_{3}$ be distinct variable symbols in $X$ and $w,w^{\prime}$ either an empty string or a regular pattern on $\Sigma\cup X$.
  %\begin{enumerate}
  %  \item[(b1)] $q=q_{1}AwBw^{\prime}Cq_{2}$, where $\{ A,B,C \} = \{ y_{1}a, dy_{2}, y_{3}c \}$.
  %  \item[(b2)] $q=q_{1}AwBq_{2}$, where $\{ A,B \} = \{ dy_{1}a, y_{3}c \}$.
  %  \item[(b3)] $q=q_{1}AwBq_{2}$, where $\{ A,B \} = \{ dy_{2}c, y_{1}a \}$.
  %  \item[(b4)] $q=q_{1}AwBq_{2}$, where $\{ A,B \} = \{ y_{1}ay_{2}, y_{3}c \}$ ($a = d$).
  %  \item[(b5)] $q=q_{1}Aq_{2}$, where $A = y_{1}ay_{2}c$ ($a = d$).
  %\end{enumerate}
%
%\noindent
%In the case of (b1), suppose that $(A, B, C) = (dy_{2}, y_{1}a, y_{3}c)$, i.e., $q=q_{1}dy_{2}wy_{1}aw^{\prime}y_{3}cq_{2}$ holds.
...
%
%  In the same way, in the cases (b2)--(b5) and also in all cases of (c), the contradiction is derived from Theorem 2.
%  From this, the cases (b) and (c) are not possible. 
  %
