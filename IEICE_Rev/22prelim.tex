...
%Sato et al.\cite{Sato1} showed that 2 is sufficient for the number $n$ in the theorem above, under the assumption that the number of constants is not less than $2k-1$.
%Theorem 20 in \cite{Sato1} showed that if the class $\RPatkei$ has compactness with respect to containment if and only if $\sharp\Sigma \geq 2k-1$, under the assumption of $\sharp\Sigma \geq 3$. 
%This means that 2 is sufficient for the number $n$ in the theorem above, under the assumption that the number of constants is not less than $2k-1$.
...
\begin{lem}[Sato et al.(Lemma 10, \cite{Sato1})]\label{Sato1:Lemma10}
    ...
    Let $p$, $p_{1}$, $p_{2}$, $q$ be regular patterns in $\RPat$ and $x$ a variable symbol such that  $p = p_{1}xp_{2}$ holds.
...
\begin{lem}[Sato et al.(Lemma 13, \cite{Sato1})]\label{Sato1:Lemma13}
    ...
    \[
        \begin{tabular}{ll}
            $\mathrm{(i)}~p_{1}Aw \preceq q_{1}$,  \\ %p_{1} \preceq q_{1}$
...
\begin{thm}[Sato et al.(Theorem 10, \cite{Sato1})]\label{Sato1:Theorem10}%\label{定理10}
    Suppose $\sharp\Sigma \geq 2k+1$.
...
The following example \ref{counter-example4Theorem10} shows  that Theorem~\ref{Sato1:Theorem10} does not hold if $\sharp\Sigma\leq 2k$ holds.%\ref{定理10}.
...
\begin{ex}\label{counter-example4Theorem10}
    ...
    %Let $p$ be a regular pattern in $\RPat$ and 
...
From Theorem~\ref{Sato1:Theorem10},%\ref{定理10}, 
