...
For two strings $p,q$, we denote by $p\cdot q$ the string obtained from $p$ and $q$ by concatenating $q$ after $p$.
...
(1) $\theta$ is a homomorphism with respect to string concatenation, that is, for patterns $p$ and $q$,
$\theta(p \cdot q) = \theta(p) \cdot \theta(q)$ holds,
...
The class of all non-empty finite subsets of $\Pat$ is denoted by $\Patplus$, that is,
$\Patplus = \{P \subseteq \Pat \mid 0 < \sharp P < \infty\}$.
For a positive integer $k$, the class of non-empty sets consisting of at most $k$ patterns, that is,
