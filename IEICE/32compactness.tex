...
This contradicts the assumption. Thus, for any $i$ ($i=1, \ldots , k$), $\sharp A_{i} \le 1$ and $\sharp B_{i} \le 1$ hold.
...
\begin{enumerate}
\item[\textrm(1-1)] 
When $\sharp A = \sharp \tilde{B} = k$ with $\sharp( A^{\prime} \cap B^{\prime} ) = 0$, w.l.o.g., we let $A'=\{a_{k+1},a_{k+2}\}$ and $B'=\{a_{1},a_{2}\}$ (see Fig.~\ref{グラフ比較} (1-1)). Moreover, since $\sharp A = \sharp B = \sharp Q$, w.l.o.g., we suppose that there is a permutation $\sigma$ such that for all $i = 1,\ldots,k$, $p \{ x:=a_{i}y \} \preceq q_{i}$ and $p \{ x:=ya_{\sigma(i)+2} \} \preceq q_{i}$.
Let $R=\{a_{k+1}a_{1}, a_{k+1}a_{2}, a_{k+2}a_{1}, a_{k+2}a_{2}\}$.
For each $r \in R$, there exists an index $i(r)$ $(1\le i(r)\le k)$ such that $p \{x := r\} \preceq q_{i(r)}$ holds.
For $a_{k+1}a_{1}\in R$, $p \{ x:=a_{i(a_{k+1}a_{1})}y \} \preceq q_{i(a_{k+1}a_{1})}$, $p \{ x:=ya_{\sigma(i(a_{k+1}a_{1}))+2} \} \preceq q_{i(a_{k+1}a_{1})}$, and $p \{ x:=a_{k+1}a_{1} \} \preceq q_{i(a_{k+1}a_{1})}$.
If $3\le i(a_{k+1}a_{1})\le k$, since $a_{i(a_{k+1}a_{1})} \not= a_{1}$, from Lemms~\ref{両方} and \ref{片方}, we get $p \{x := xy\} \preceq q_{i(r)}$. This contradicts the assumption.
Similarly for any $r\in R$, we have $i(r) = 1$ or $i(r) = 2$.



If $\sigma(1) \not= k - 1$, since $p \{ x:=ya_{\sigma(1) + 2} \} \preceq q_{1}$ and $p \{ x:=a_{1}y \} \preceq q_{i}$, $p \{x := a_{k+1}a_{1}\} \not\preceq q_{1}$ holds. Nevertheless, from Lemma~\ref{片方}, we have $p \{x := xy\} \preceq q_{1}$. It contradicts the assumption.
Thus, if $p \{x := a_{k+1}a_{1}\} \preceq q_{1}$, then $\sigma(1) = k - 1$ holds.
By the same reason, if $p \{x := a_{k+2}a_{1}\} \preceq q_{1}$, then $\sigma(1) = k$ holds.
Therefore, since $\sigma$ is a permutation, either $p \{x := a_{k+1}a_{1}\} \preceq q_{1}$ or $p \{x := a_{k+2}a_{1}\} \preceq q_{1}$ holds.
Similarly either $p \{x := a_{k+1}a_{2}\} \preceq q_{2}$ or $p \{x := a_{k+2}a_{2}\} \preceq q_{2}$ holds.


if there exists $q \in Q'$ such that $p \{ x:=w \} \preceq q$ holds, from Lemma~\ref{追加部分}, $p \{ x:=xy \} \preceq q$ holds. This contradicts the assumption.
If there exists $q \in Q'$ such that $p \{ x:=a_{1}y \} \preceq q$ and $p \{ x:=ya_{k+1} \} \preceq q$ hold, from Corollary~\ref{両方}, $p \{ x:=a_{k+1}a_{1} \} \preceq q$ may hold.
...
we let $p \{ x:=ya_{i'} \} \preceq q_{i}$ and $p \{ x:=a_{i''}y \} \preceq q_{i}$, $(i,i',i''=3,\ldots,k)$.
Moreover, we let $p \{ x:=a_{i}y \} \preceq q_{i}$, and $p \{ x:=ya_{k+i} \} \preceq q_{i}$ $(i=1,2)$  (see Fig.~\ref{グラフ比較} (1-1)).
...
\noindent
From Corollary~\ref{両方}, $p \{ x:=a_{k+1}a_{1} \} \preceq q_{1}$(図\ref{系との関係})，$p \{ x:=a_{k+2}a_{2} \} \preceq q_{2}$となる$q_{1},~q_{2}$が考えられる．
...
\mywidth=\textwidth
\divide\mywidth by 5
\multiply\mywidth by 4

\begin{figure*}[t]
\centering
\includegraphics[width=\mywidth]{figs/Exam-Cases-(2).png}
%\vspace*{-1cm}
\caption{Examples of (2-1), (2-2), (2-3), and (2-4)}
\label{(2)におけるパターン}
\end{figure*}
