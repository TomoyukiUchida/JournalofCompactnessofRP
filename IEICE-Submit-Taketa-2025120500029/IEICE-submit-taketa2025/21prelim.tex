...
\begin{dfn}
    ...
    We say that $S$ is a {\color{black}\textit{characteristic set}} of $L$ within $\C$
    if for any $L' \in \C$,
...
\begin{lem}[Sato et al.(Lemma 10, \cite{Sato1})]\label{Sato1:Lemma10}
    {\color{black} Let $\Sigma$ be an alphabet with $\sharp \Sigma \geq 3$ and $x$ a variable symbol in $X$.
...
\begin{thm}[Sato et al.(Theorem {\color{black} 11}, \cite{Sato1})]\label{Sato1:Theorem11}
    Let $k$ be a positive integer {\color{black} with $k\ge 1$ and $\Sigma$  an alphabet with $\sharp\Sigma \geq 2k+1$}.
    For {\color{black}$P \in \RPatplus_{\Sigma\cup X}$} and {\color{black}$Q \in \RPatkei_{\Sigma\cup X}$}, 
...
\begin{col}[Sato et al.(Corollary 12, \cite{Sato1})]
    ...
    For two regular patterns {\color{black}$p,q\in\RPat_{\Sigma\cup X}$},
...
{\color{black} The following lemma demonstrates that Theorem~\ref{Sato1:Theorem11} does not hold in general when $\sharp\Sigma \leq 2k$.
...
\begin{lem}[Sato et al.(Lemma 13, \cite{Sato1})]\label{Sato1:Lemma13}
    Let $\Sigma$ be an alphabet with $\sharp\Sigma \geq 3$.
    Let $p_{1}$, $p_{2}$, $q_{1}$, $q_{2}$ be regular patterns in {\color{black}$\RPat_{\Sigma\cup X}$} and $x$ a variable symbol.
    Let $a$, $b$ be constant symbols in $\Sigma$ with $a \neq b$ and $w$ a string {\color{black}in} $\Sigma^{\ast}$.
    Let $p = p_{1}AwxwBp_{2}$ and $q = q_{1}AwBq_{2}$ be regular patterns in {\color{black}$\RPat_{\Sigma\cup X}$} {\color{black}satisfying} the following three conditions:
    \[
        \begin{tabular}{ll}
...
{\color{black} The following {\color{black}example} illustrates the failure of Theorem~\ref{Sato1:Theorem11} {\color{black}under $\sharp\Sigma \leq 2k$}, in accordance with Lemma~\ref{Sato1:Lemma13}.} 

\begin{ex}[{\color{black}Example~\ref{counter-example4Theorem11}, \cite{Sato1}}]\label{counter-example4Theorem11}
    Let $k$ be a positive integer and $\Sigma= \{a_{1},\ldots,a_{k},b_{1},\ldots,b_{k}\}$.
    Let $w_{1},\ldots,w_{k}$ be regular patterns in {\color{black}$\RPat_{\Sigma\cup X}$} such that 
    $w_{k} = \varepsilon$ and for $i=1,2,\ldots,k-1$, $w_{i} = w_{i+1}b_{i+1}a_{i+1}w_{i+1}$.
    Let $p$, $q_{1},\ldots,q_{k}$ be regular patterns in {\color{black}$\RPat_{\Sigma\cup X}$} such that
    $p = x_{1}a_{1}w_{1}xw_{1}b_{1}x_{2}$ and for $i=1,2,\ldots,k$, $q_{i} = x_{1}a_{i}w_{i}b_{i}x_{2}$.
    Let $Q$ be a set $\{q_{1},\ldots,q_{k}\}$ in {\color{black}$\RPatkei_{\Sigma\cup X}$}.
    ...
    we easily see that $w_{1}=(w_{i}b_{i})w^{(i)}={\color{black}w^{\prime(i)}}(a_{i}w_{i})$ for some strings $w^{(i)}$ and $w^{\prime(i)}$.
