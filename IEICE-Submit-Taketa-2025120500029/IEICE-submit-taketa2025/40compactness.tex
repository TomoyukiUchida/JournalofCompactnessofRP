...
we show that 
$\RPat^{k}$ has compactness with respect to {\color{black} language } containment.
...
the set $\mathcal{C}$ {\color{black} is said} to have {\it compactness with respect to {\color{black} language } containment}
{\color{black} if the following condition holds:
...
\begin{figure*}[t]
  \begin{center}
    ...
    \caption{Let $\Sigma=\{a,b,c,d,e,f,g\}$ {\color{black} and } $Q=\{q_1,q_2,q_3,q_4,q_5\}$. 
...
\begin{figure}[t]
  \begin{center}
    ...
    \caption{In the left figure, we aggregate all edges occurring in Fig.~\ref{fig:lem8eachreg}. For all $w=a'b'\in A'\cdot B'$, there must be a regular pattern $q_i$ $(1\leq i\leq 5$) {\color{black} satisfying} $p \{ x:=w \} \preceq q_i$.}\label{fig:lem8totalreg}
  \end{center}
\end{figure}
...
\begin{figure*}[t]
  \begin{center}
    ...
    \caption{Let $\Sigma=\{a,b,c,d,e,f,g\}$ {\color{black} and} $Q=\{q_6,q_7,q_8,q_9,q_{10}\}$. From these figures, we get $\ell_A=1$, $\ell_B=1$, $Q^{(\bot,\bot)}=Q^{(\bot,\cdot)}=Q^{(\cdot,\bot)}=\emptyset$, and $Q^{(\cdot,\cdot)}=Q$.}\label{fig:lem8eachreg-suppl}
  \end{center}
\end{figure*}
...
{\color{black}Consider a regular pattern $p\in \RPat_{\Sigma\cup X}$ that contains} a variable symbol {\color{black}$x\in X$}, and let $Q \in \RPat^{k}$.
{\color{black}Assume that for any string $w \in \Sigma^{\ast}$ with $|w|=2$, there exists a regular pattern $q_{w} \in Q$ such that $p \{ x:=w \} \preceq q_{w}$.
...
Since $\sharp \Sigma = k + 2$ and $\sharp Q = k$, {\color{black} both} $\sharp A' \geq 2$ and $\sharp B' \geq 2$ hold.
...
\end{align*}
From the condition of this lemma, for any string $w \in \Sigma^{\ast}$ with $|w|=2$, there exists a regular pattern $q_{w} \in Q$ {\color{black}with} $p \{ x:=w \} \preceq q_{w}$.
In particular, for $w=a'b'\in A'\cdot B'$, we must have $q_{w} \in Q$ {\color{black}satisfying} $p \{ x:=w \} \preceq q_{w}$ (Fig.~\ref{fig:lem8totalreg}).
It is easy to see that if $w \in (A\cdot B) \cup (A'\cdot B) \cup (A\cdot B')$, there exists a regular pattern $q_{w} \in Q^{(\bot,\cdot)} \cup Q^{(\cdot,\bot)} \cup Q^{(\cdot,\cdot)}$ {\color{black}with} $p \{ x:=w \} \preceq q_{w}$.
...
{\color{black}From Lemma~\ref{Sato1:Lemma10}, for any $i$ ($1\leq i\leq 5$), if either $\sharp (W\cap \{a_{i}c \mid c \in \Sigma\})\geq 3$ or $\sharp (W \cap \{cb_{i} \mid c \in \Sigma\}) \geq 3$ holds, then $a_{i}\in A(q)$ or $b_{i}\in B(q)$.
From the definitions of $A'$ and $B'$, {\color{black}we have} that,  
for any $i$ ($1\leq i\leq 5$), $\sharp (W \cap \{a_{i}c \mid c \in \Sigma\}) \leq 2$ and $\sharp (W \cap \{cb_{i} \mid c \in \Sigma\}) \leq 2$.}
{\color{black} Then, it is proved} that there are $3$ strings $a_{i_{1}}b_{i_{1}},a_{i_{2}}b_{i_{2}},a_{i_{3}}b_{i_{3}}\in W$ such that $a_{i_{j}} \not= a_{i_{j^{\prime}}}$ and $b_{i_{j}} \not= b_{i_{j^{\prime}}}$ for any $i_{j},i_{j^{\prime}}$ ($i_{j}\not= i_{j^{\prime}}, {\color{black}1\leq j<j^{\prime}\leq 3}$).
...
Because, for any $i$ ($1\leq i\leq 3$), $\sharp(W \cap \{a_{i}c \mid c \in \Sigma\}) \leq 2$ and  $\sharp (W \cap \{cb_{i} \mid c \in \Sigma\}) \leq 2$, {\color{black} it is proved} that there are $2$ strings $a_{i_{1}}b_{i_{1}},a_{i_{2}}b_{i_{2}}\in W$ such that $a_{i_{1}} \not= a_{i_{2}}$ and $b_{i_{1}} \not= b_{i_{2}}$.
...
\end{align*}
We show the {\color{black}following} two claims {\color{black}concerning} $Q_{1}^{(\cdot,\cdot)}$ and $Q_{2}^{(\cdot,\cdot)}$:
...
Similarly, the case that $\sigma_{A}(q)\in B'$ and $\sigma_{B}(q)\in A$ is {\color{black}proved}. (\textit{End of Proof of Claim} 4)
...
\noindent
{\color{black} We give an example in Fig.~\ref{fig:lem8eachreg-suppl} and Fig.~\ref{fig:lem8totalreg-suppl}.}
...
= & \ell_B + 2 \geq 2.
\end{align*}
Similarly, the case when $\sharp A \leq k$ and $\sharp B = k$ is {\color{black}proved}.
(\textit{End of Proof of Claim} {\color{black}6})
...
{\color{black} For regular patterns $p$ and $q$,
assume that there exists a constant symbol $a \in \Sigma$ such that $p \{ x := a \} \preceq q$ and $p \{ x := xy \} \preceq q$, where $y$ is a variable symbol that does not occur in {\color{black}$p$}.
Then $p \preceq q$.}
\end{lem}
